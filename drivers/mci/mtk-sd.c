...
#define to_msdc(h) container_of(h, struct msdc_host, mci)

static inline struct device *to_msdc_dev(struct msdc_host *host)
{
	return &host->mci.mci->dev;
}
...
static u32 msdc_cmd_find_resp(struct msdc_host *host, struct mci_cmd *cmd)
{
	u32 resp;
...
static int msdc_start_command(struct msdc_host *host, struct mci_cmd *cmd,
			      struct mci_data *data)
{
	u32 rawcmd;
	u32 status;
...
static int msdc_pio_transfer(struct msdc_host *host, struct mci_data *data)
{
	u32 size;
	int ret;
...
static int msdc_dma_transfer(struct msdc_host *host, struct mci_data *data)
{
	u32 size, status;
	int ret;
	const void *buf;
...
static int msdc_start_data(struct msdc_host *host, struct mci_data *data)
{
	int ret;
...
static void msdc_set_buswidth(struct msdc_host *host, enum mci_bus_width width)
{
...
static void msdc_set_mclk(struct msdc_host *host, enum mci_timing timing, u32 hz)
{
	struct device *dev = to_msdc_dev(host);
	u32 mode;
	u32 div;
	u32 sclk;
	u32 reg;
	...
	if (timing == MMC_TIMING_UHS_DDR50 || timing == MMC_TIMING_MMC_DDR52 ||
		...
		if (timing == MMC_TIMING_MMC_HS400 && hz >= (host->src_clk_freq >> 1)) {
			if (host->dev_comp->clk_div_bits == 8)
...
static void msdc_ops_set_ios(struct mci_host *mci, struct mci_ios *ios)
{
	struct msdc_host *host = to_msdc(mci);

	msdc_set_buswidth(host, ios->bus_width);

	if (host->mclk != ios->clock || ios->timing != host->timing)
		msdc_set_mclk(host, mci->ios.timing, ios->clock);
}

static int msdc_ops_get_cd(struct mci_host *mci)
{
	struct msdc_host *host = to_msdc(mci);
	u32 val;
...
static int msdc_ops_get_wp(struct mci_host *mci)
{
	struct msdc_host *host = to_msdc(mci);
	...
	return !gpiod_get_value(host->gpio_wp);
}
...
static int hs400_tune_response(struct msdc_host *host, u32 opcode)
{
	struct device *dev = to_msdc_dev(host);
	u32 cmd_delay  = 0;
	struct msdc_delay_phase final_cmd_delay = { 0, };
	enum mci_timing	timing = host->mci.ios.timing;
	u8 final_delay;
	...
	for (i = 0; i < PAD_DELAY_MAX; i++) {
		...
		for (j = 0; j < 3; j++) {
			cmd_err = mmc_send_tuning(host->mci.mci, opcode);
			if (!cmd_err) {
				cmd_delay |= (1 << i);
			} else {
				cmd_delay &= ~(1 << i);
...
static int msdc_tune_response(struct msdc_host *host, u32 opcode)
{
	struct device *dev = to_msdc_dev(host);
...
static int msdc_tune_data(struct msdc_host *host, u32 opcode)
{
	struct device *dev = to_msdc_dev(host);
	enum mci_timing	timing = host->mci.ios.timing;
	u32 rise_delay = 0, fall_delay = 0;
	...
	for (i = 0; i < PAD_DELAY_MAX; i++) {
		...
		ret = mmc_send_tuning(host->mci.mci, opcode);
		if (!ret) {
			rise_delay |= (1 << i);
		} else {
			/* in this case, retune response is needed */
			ret = msdc_tune_response(host, opcode);
			if (ret)
				break;
		}
	}
 ...
 */
static int msdc_tune_together(struct msdc_host *host, u32 opcode)
{
	struct device *dev = to_msdc_dev(host);
	u32 rise_delay = 0, fall_delay = 0;
	struct msdc_delay_phase final_rise_delay, final_fall_delay = { 0, };
...
static __maybe_unused int msdc_execute_tuning(struct mci_host *mci, uint opcode)
{
	struct msdc_host *host = to_msdc(mci);
	struct device *dev = to_msdc_dev(host);
	enum mci_timing	timing = host->mci.ios.timing;
	...
	if (host->dev_comp->data_tune && host->dev_comp->async_fifo) {
		ret = msdc_tune_together(host, opcode);
		if (ret == -EIO) {
	...
	if (timing == MMC_TIMING_MMC_HS400)
		ret = hs400_tune_response(host, opcode);
	else
		ret = msdc_tune_response(host, opcode);
	if (ret == -EIO) {
	...
	if (timing != MMC_TIMING_MMC_HS400) {
		ret = msdc_tune_data(host, opcode);
		if (ret == -EIO) {
...
static int msdc_card_busy(struct mci_host *mci)
{
	struct msdc_host *host = to_msdc(mci);
	u32 status = readl(&host->base->msdc_ps);

	/* only check if data0 is low */
	return !(status & MSDC_PS_DAT0);
}

static int msdc_card_init(struct mci_host *mci, struct device *dev)
{
	struct msdc_host *host = to_msdc(mci);
...
struct mci_ops msdc_ops = {
	.init = msdc_card_init,
	.set_ios = msdc_ops_set_ios,
	.send_cmd = msdc_ops_send_cmd,
	.card_present = msdc_ops_get_cd,
	.card_write_protected = msdc_ops_get_wp,
...
static int msdc_drv_probe(struct device *dev)
{
	...
	host->dev_comp = device_get_match_data(dev);

	host->base = dev_platform_ioremap_resource(dev, 0);
	if (IS_ERR(host->base))
		return PTR_ERR(host->base);

	if (host->dev_comp->needs_top_base) {
		host->top_base = dev_platform_ioremap_resource(dev, 1);
	...
	host->src_clk_cg = clk_get_optional(dev, "source_cg");
	if (IS_ERR(host->src_clk_cg))
		return PTR_ERR(host->src_clk_cg);

	host->axi_cg_clk = clk_get_optional(dev, "axi_cg");
	if (IS_ERR(host->axi_cg_clk))
		return PTR_ERR(host->axi_cg_clk);

	host->ahb_cg_clk = clk_get_optional(dev, "ahb_cg");
	if (IS_ERR(host->ahb_cg_clk))
		return PTR_ERR(host->ahb_cg_clk);

	host->gpio_wp = gpiod_get_optional(dev, "wp-gpios", GPIOD_IN);
	host->gpio_cd = gpiod_get_optional(dev, "cd-gpios", GPIOD_IN);

	of_property_read_u32(np, "hs400-ds-delay", &host->hs400_ds_delay);

	of_property_read_u32(np, "mediatek,hs200-cmd-int-delay",
			     &host->hs200_cmd_int_delay);

	of_property_read_u32(np, "write_int_delay", &host->hs200_write_int_delay);

	of_property_read_u32(np, "mediatek,latch-ck", &host->latch_ck);

	host->r_smpl = of_property_read_bool(np, "mediatek,hs400-cmd-resp-sel-rising");
	...
	if (host->dev_comp->clk_div_bits == 8)
		mci->f_min = host->src_clk_freq / (4 * 255);
	else
		mci->f_min = host->src_clk_freq / (4 * 4095);

	if (mci->f_min < MIN_BUS_CLK)
		mci->f_min = MIN_BUS_CLK;

	if (mci->f_max < mci->f_min || mci->f_max > host->src_clk_freq)
		mci->f_max = host->src_clk_freq;
...
static const struct of_device_id msdc_ids[] = {
	...
	{ .compatible = "mediatek,mt7623-mmc", .data = &mt7623_compat },
	{ .compatible = "mediatek,mt7986-mmc", .data = &mt7986_compat },
	{ .compatible = "mediatek,mt7987-mmc", .data = &mt7987_compat },
	{ .compatible = "mediatek,mt7981-mmc", .data = &mt7981_compat },
	{ .compatible = "mediatek,mt8512-mmc", .data = &mt8512_compat },
...
static struct driver mt_msdc_driver = {
	.probe = msdc_drv_probe,
	.name = "mtk-msdc",
	.of_match_table = msdc_ids,
};
device_platform_driver(mt_msdc_driver);
