 ...
 *
 * Based on Linux pinctrl-mtk-common-v2:
...
static const struct mtk_drive_desc mtk_drive[] = {
	[DRV_GRP0] = { 4, 16, 4, 1 }, [DRV_GRP1] = { 4, 16, 4, 2 },
	[DRV_GRP2] = { 2, 8, 2, 1 },  [DRV_GRP3] = { 2, 8, 2, 2 },
	[DRV_GRP4] = { 2, 16, 2, 1 }, [DRV_GRP5] = { 4, 32, 4, 1 },
};

struct mtk_pinctrl *to_mtk_pinctrl(struct pinctrl_device *pdev)
{
	return container_of(pdev, struct mtk_pinctrl, pctl_dev);
}
...
static int mtk_hw_pin_field_lookup(struct mtk_pinctrl *hw,
				   const struct mtk_pin_desc *desc, int field,
				   struct mtk_pin_field *pfd)
{
	...
	if (hw->soc->reg_cal && hw->soc->reg_cal[field].range) {
		...
		dev_dbg(hw->dev, "Not support field %d for this soc\n", field);
		return -ENOTSUPP;
	}
	 ...
	 */
	bits = c->fixed ? c->s_bit :
			  c->s_bit + (desc->number - c->s_pin) * (c->x_bits);
				    ...
				    struct mtk_pin_field *pf, int *value)
{
	...
	l = (mtk_r32(hw, pf->index, pf->offset) >> pf->bitpos) &
	    (BIT(nbits_l) - 1);
 ...
 * Detect them so we can ignore them.
 */
		     ...
		     int field, int *value)
{
	...
	if (!pf.next)
		*value = (mtk_r32(hw, pf.index, pf.offset) >> pf.bitpos) &
			 pf.mask;
	else
				 ...
				 const struct mtk_pin_desc *desc)
{
	...
	err = mtk_hw_set_value(hw, desc, PINCTRL_PIN_REG_PU, MTK_DISABLE);
	if (err)
		return err;

	err = mtk_hw_set_value(hw, desc, PINCTRL_PIN_REG_PD, MTK_DISABLE);
	if (err)
		return err;
			 ...
			 const struct mtk_pin_desc *desc, bool pullup)
{
	...
	err = mtk_hw_set_value(hw, desc, PINCTRL_PIN_REG_PD, !!(arg & 2));
	if (err)
		return err;
...
int mtk_pinconf_bias_disable_set_rev1(struct mtk_pinctrl *hw,
				      const struct mtk_pin_desc *desc)
{
	return mtk_hw_set_value(hw, desc, PINCTRL_PIN_REG_PULLEN, MTK_DISABLE);
}
			      ...
			      const struct mtk_pin_desc *desc, bool pullup)
{
	...
	err = mtk_hw_set_value(hw, desc, PINCTRL_PIN_REG_PULLEN, MTK_ENABLE);
	if (err)
		return err;
...
static int mtk_hw_pin_rsel_lookup(struct mtk_pinctrl *hw,
				  const struct mtk_pin_desc *desc, u32 pullup,
				  u32 arg, u32 *rsel_val)
{
			       ...
			       u32 arg)
{
	...
	if (err)
		dev_err(hw->dev, "Invalid pull argument for pin %s\n",
			desc->name);
			  ...
			  const struct mtk_pin_desc *desc, u32 arg)
{
	...
	if ((arg >= tb->min && arg <= tb->max) && !(arg % tb->step)) {
		...
		err = mtk_hw_set_value(hw, desc, PINCTRL_PIN_REG_E4, arg & 0x1);
		if (err)
			return err;
			     ...
			     u32 arg)
{
	...
	err = mtk_hw_set_value(hw, desc, PINCTRL_PIN_REG_R1, !!(arg & 2));
	if (err)
		return 0;
