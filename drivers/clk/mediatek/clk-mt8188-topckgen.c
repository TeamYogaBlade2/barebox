...
static int clk_mt8188_topck_probe(struct device *dev)
{
	...
	r = mtk_clk_register_muxes(dev, top_mtk_muxes,
				   ARRAY_SIZE(top_mtk_muxes), node,
				   &mt8188_clk_lock, top_clk_data);
	if (r)
		goto unregister_factors;

	hw = clk_hw_register_mux(dev, "mfg_ck_fast_ref", mfg_fast_ref_parents,
	...
	r = mtk_clk_register_composites(dev, top_adj_divs,
					ARRAY_SIZE(top_adj_divs), base,
					&mt8188_clk_lock, top_clk_data);
	if (r)
		goto unregister_muxes;

	r = mtk_clk_register_gates(dev, node, top_clks,
	...
	dev_set_drvdata(dev, top_clk_data);
...
static void clk_mt8188_topck_remove(struct device *dev)
{
	struct clk_hw_onecell_data *top_clk_data = dev_get_drvdata(dev);
	struct device_node *node = dev->of_node;
