...
static int __mtk_clk_simple_probe(struct device *dev,
				  struct device_node *node)
{
	...
	if (mcd->mux_clks) {
		r = mtk_clk_register_muxes(dev, mcd->mux_clks,
	...
	if (mcd->divider_clks) {
		r = mtk_clk_register_dividers(dev,
					      mcd->divider_clks,
	...
	if (mcd->clks) {
		r = mtk_clk_register_gates(dev, node, mcd->clks,
	...
	if (mcd->rst_desc) {
		r = mtk_register_reset_controller_with_dev(dev,
							   mcd->rst_desc);
		if (r)
			goto unregister_clks;
	}
...
static void __mtk_clk_simple_remove(struct device *dev,
				   struct device_node *node)
{
	struct clk_hw_onecell_data *clk_data = dev_get_drvdata(dev);
	const struct mtk_clk_desc *mcd = device_get_match_data(dev);
...
int mtk_clk_pdev_probe(struct device *dev)
{
	struct device_node *node = dev->parent->of_node;

	return __mtk_clk_simple_probe(dev, node);
}
EXPORT_SYMBOL_GPL(mtk_clk_pdev_probe);

int mtk_clk_simple_probe(struct device *dev)
{
...
void mtk_clk_pdev_remove(struct device *dev)
{
	struct device_node *node = dev->parent->of_node;

	__mtk_clk_simple_remove(dev, node);
}
EXPORT_SYMBOL_GPL(mtk_clk_pdev_remove);

void mtk_clk_simple_remove(struct device *dev)
{
	__mtk_clk_simple_remove(dev, dev->of_node);
}
