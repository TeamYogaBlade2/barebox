...
static int clk_mt8188_apmixed_probe(struct device *dev)
{
	...
	r = mtk_clk_register_gates(dev, node, apmixed_clks,
...
static void clk_mt8188_apmixed_remove(struct device *dev)
{
	struct device_node *node = dev->of_node;
...
static struct driver clk_mt8188_apmixed_drv = {
...
postcore_platform_driver(clk_mt8188_apmixed_drv);
